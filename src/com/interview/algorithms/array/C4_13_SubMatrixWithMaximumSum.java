...
public class C4_13_SubMatrixWithMaximumSum {
  ...
  private final Hashtable<String, Integer> sums = new Hashtable<String, Integer>();
  private int maxSum = Integer.MIN_VALUE;
  private int maxSumLeftUpperPointRow;
  private int maxSumLeftUpperPointColumn;
  private int maxSumRows, maxSumColumns;
  ...
  public void findMatrixWithMaxSum(int[][] array) {
    ...
    for (int subMatrixRows = rows; subMatrixRows >= 1; subMatrixRows--)
      for (int subMatrixColumns = columns; subMatrixColumns >= 1; subMatrixColumns--)
        findMatrixWithMaxSum(array, subMatrixRows, subMatrixColumns);//O(N*M)
    ...
    for (int rowLeftUpper = 0; rowLeftUpper < arrayRows; rowLeftUpper++) {
      ...
      for (int columnLeftUpper = 0; columnLeftUpper < arrayColumns; columnLeftUpper++) {
        ...
        if (columnRightBottom >= arrayColumns)
          break;
        int matrixSum = this.calculateMatrixSum(rowLeftUpper, columnLeftUpper,
            rowRightBottom, columnRightBottom);
        System.out.println(String.format("Left Upper Point = (%s, %s), Right Bottom Point = (%s, %s), Sum = %s",
            rowLeftUpper, columnLeftUpper,
            rowRightBottom, columnRightBottom, matrixSum));
        if (matrixSum > maxSum) {
          maxSum = matrixSum;
          maxSumLeftUpperPointColumn = columnLeftUpper;
          maxSumLeftUpperPointRow = rowLeftUpper;
          maxSumRows = rows;
          maxSumColumns = columns;
        }
      }
    }
  }
  ...
  private int calculateMatrixSum(int rowLeftUpper, int columnLeftUpper,
    ...
    int sum = sums.get(rowRightBottom + "," + columnRightBottom).intValue();
    if (rowLeftUpper == 0 && columnLeftUpper == 0 && rowRightBottom >= 0 && columnRightBottom >= 0) {
      ...
      /* Any sub matrix whose left upper point's column is 0
			 ...
			 * % % % %
			 */
      sum -= sums.get((rowLeftUpper - 1) + "," + columnRightBottom).intValue();
      ...
      /* Any sub matrix whose left upper point's row is 0
			 ...
			 * % % % %
			 */
      sum -= sums.get(rowRightBottom + "," + (columnLeftUpper - 1));
    } else {
      /* Any sub matrix whose left upper point is Not on the left or upper edge of the matrix
			 ...
			 * % 5 6 %
			 */
      sum = sums.get(rowRightBottom + "," + columnRightBottom).intValue() -
          sums.get((rowLeftUpper - 1) + "," + columnRightBottom).intValue() -
          sums.get(rowRightBottom + "," + (columnLeftUpper - 1)).intValue() +
          sums.get((rowLeftUpper - 1) + "," + (columnLeftUpper - 1)).intValue();
    }
    return sum;
  }
