...
public class C1_24_ExpressionValidation {
  ...
  public boolean validate(String expression) {
    ...
    if ("".equals(expression.trim())) // empty string are valid expressions
      return true;
    C1_24_ExpressionValidation.Stack stack = new C1_24_ExpressionValidation.Stack();
    for (int i = 0; i < expression.length(); i++) {
      ...
      if (current == ')') {
        ...
        if (!validateBasicExpression(subExpr))
          return false;
        else
    ...
    return validateBasicExpression(stack.toString());
  }
  ...
  private boolean validateBasicExpression(String basicExpression) {
    ...
    for (int i = 0; i < basicExpression.length(); i++) {
      ...
      if (isOperator(previous) && isOperator(current) || isParentheses(current))
        return false;
      previous = current;
    }
    return true;
