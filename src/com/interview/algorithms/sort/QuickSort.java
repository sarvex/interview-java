...
public class QuickSort {
  ...
  public int[] sort(int[] array, int begin, int end) {
    if (begin <= end) {
      QuickSort.State state = this.partition(array, begin, end);
      array = state.array;
   ...
   */
  private QuickSort.State partition(int[] array, int begin, int end) {
    int i = begin - 1;
    int value = array[end];
    ...
    QuickSort.State state = new QuickSort.State();
    state.array = switchElements(array, i + 1, end); // switch array[end] to section 1)
    state.pivot = i + 1;
